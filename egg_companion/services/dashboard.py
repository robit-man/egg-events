...
function voiceChoices(state){if(!catalog)return;const voice=state.telemetry?.voice||{},tts=(catalog.tts?.models||[]).filter(model=>model.enabled!==false).map(model=>({id:model.id,label:`${model.label||model.id} · ${model.backend||'tts'}`})),asr=(catalog.asr?.models||[]).map(model=>({id:model.id,label:model.id+(model.isActive?' · active':'')}));select('voice_model',tts,voice.tts_model);select('asr_model',asr,voice.asr_model);const chosen=$('#voice [name=voice_model]').value,entry=(catalog.tts?.models||[]).find(model=>model.id===chosen),voices=(entry?.voices?.length?entry.voices:(chosen==='supertonic'?(catalog.supertonic?.options?.voices||[]):[])).map(id=>({id,label:id}));select('voice_name',voices,voice.tts_voice||catalog.supertonic?.settings?.voiceName);$('#voice [name=voice_name]').disabled=!voices.length}
