...
<body>
      ...
      <div class="content">
        ...
        <section class="page" data-page="/cognition">
          ...
          <div class="grid">
            ...
            <article class="card span-12"><div class="card-header"><div><h3 class="card-title">Environmental pondering</h3><p class="card-note">Event-driven pixel grounding, novelty falloff, memory recall, private reflection, and model-chosen speech or silence</p></div><div id="environmental-cognition-state" class="badge-row"></div></div><div id="environmental-cognition-ledger" class="table-wrap"></div></article>
  ...
  <script>
    ...
    function renderCognition(telemetry) {
      ...
      const environment = telemetry.environmental_cognition || {}, environmentLast = environment.last || {}, environmentRecent = (environment.recent || []).slice(-16).reverse();
      ...
      $('#environmental-cognition-state').innerHTML = `<span class="badge ${environment.state === 'spoken' || environment.state === 'reflect' || environment.state === 'grounded' ? 'good' : environment.state === 'error' ? 'bad' : environment.state === 'preempted' || environment.state === 'timed_out' || environment.state === 'faded' ? 'warn' : ''}">${esc(stateLabel(environment.state || 'idle'))}</span><span class="badge">${esc(environment.pixel_wakeups || 0)} pixel wakeups</span><span class="badge">${esc(environment.grounded || 0)} grounded</span><span class="badge">${esc(environment.reflected || 0)} reflections</span><span class="badge">${esc(environment.spoken || 0)} spoken</span><span class="badge">${esc(environment.silent || 0)} silent</span><span class="badge">${esc(environment.preempted || 0)} yielded to speech</span><span class="badge">${esc(environment.timed_out || 0)} timed out</span>${environmentLast.salience == null ? '' : `<span class="badge">salience ${Math.round(Number(environmentLast.salience) * 100)}%</span>`}`;
      $('#environmental-cognition-ledger').innerHTML = table(['State','Evidence / decision','Salience','Time'], environmentRecent.map(item => [statusBadge(item.state || 'idle'), esc(item.detail || item.stimulus_id || '—'), item.salience == null ? '—' : `${Math.round(Number(item.salience) * 100)}%`, esc(formatTime(item.updated_at))]), 'No environmental changes have reached cognition yet');
