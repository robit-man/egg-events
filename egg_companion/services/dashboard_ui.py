...
<body>
  ...
  <script>
    ...
    function renderCognition(telemetry) {
      ...
      $('#environmental-cognition-state').innerHTML = `<span class="badge ${environment.state === 'spoken' || environment.state === 'reflect' || environment.state === 'grounded' ? 'good' : environment.state === 'error' ? 'bad' : environment.state === 'preempted' || environment.state === 'faded' ? 'warn' : ''}">${esc(stateLabel(environment.state || 'idle'))}</span><span class="badge">${esc(environment.pixel_wakeups || 0)} pixel wakeups</span><span class="badge">${esc(environment.grounded || 0)} grounded</span><span class="badge">${esc(environment.reflected || 0)} reflections</span><span class="badge">${esc(environment.spoken || 0)} spoken</span><span class="badge">${esc(environment.silent || 0)} silent</span><span class="badge">${esc(environment.preempted || 0)} yielded to speech</span>${environmentLast.salience == null ? '' : `<span class="badge">salience ${Math.round(Number(environmentLast.salience) * 100)}%</span>`}`;
