...
## Verbal object-memory recall and past-sighting OCR

`recall_object_memory` and `read_past_camera_text` are realtime tool-calling capabilities the conversational model can invoke directly — "where did you see my keys" or "what did that sign say" are answered from the world model's own history, not guessed. Recall ranks the speaker's own words against every entity's current `label`/`semantic_tags` by CLIP embedding similarity as the primary mechanism (`egg_companion/runtime.py`'s `_associative_object_recall`) rather than substring/regex matching, so "the thing I drink from" finds a mug labeled "red mug" as readily as the word "mug" would; a literal SQL-`LIKE` resolver (`WorldStateStore.search_property_text` in `egg_companion/world/state.py`, composed into sighting history by `WorldQuery.recall_object_sightings` in `egg_companion/world/query.py`) exists only as a fallback for when the embedding model itself is unavailable, never as normal-path behavior. Every conversational turn also receives real current date/time in context, so the model reasons out exact `since`/`until` bounds for "yesterday" or "last week" itself from that ground truth rather than being limited to a fixed keyword enum or guessing without knowing "now".

When `memory.retain_raw_media` is enabled, a real stored frame from each camera detection is linked to its world-model entity through the same `evidence_id` the legacy episodic-memory evidence table already tracks (surfaced via `world_assertions.evidence_ids_json` in `egg_companion/world/reconcile.py`). `read_past_camera_text` resolves a past sighting the same associative way, then runs the exact same advanced-OCR engine `read_current_camera_text` uses against that stored frame — reading a sign or screen seen earlier, not just the live camera — and says plainly when no stored image exists (already expired past `raw_media_retention_hours`, or the event predates `retain_raw_media`) rather than inventing text.

A burst of utterances spoken close together — or one thought ASR segments into several boundaries — is drained and consolidated into a single reasoning turn rather than processed as separate sequential requests (`_reason_about_transcript` in `runtime.py`): only the most recent utterance could ever be published anyway once a newer one has finalized, so folding them together gives the model full scope of what was actually said instead of an isolated fragment, at the cost of one inference call instead of several mostly-discarded ones. A reasoning failure — a timed-out or otherwise unavailable model call — never speaks a hardcoded fallback line; it stays silent but is fully recorded in runtime-error telemetry and the interaction log, diagnosable without ever being mistaken for a successful turn.

## Fused multi-camera voxel occupancy mapping

The four cameras are a co-located panoramic array (video0 rightmost, sweeping counter-clockwise through video1–video3, ~60° between neighbors, configured in `occupancy.camera_yaw_degrees`), not independent unrelated viewpoints — the array is modeled as sharing one optical center (no translation, only yaw), so the fused frame's origin `(0,0,0)` is, physically, the center of the ring the cameras mount around. `egg_companion/core/occupancy.py`'s `VoxelGrid` fuses every camera's monocular metric depth into one shared "egg frame" grid by rotating each camera's back-projected points by its known array yaw before voxelizing, rather than keeping four disconnected local reconstructions. Depth itself comes from Depth Anything 3 (`DA3METRIC-LARGE`), run on demand as a bounded-lifetime subprocess in a sibling project's venv (`egg_companion/adapters/depth.py`) — this Jetson doesn't have headroom to keep a ~4GB model resident alongside vision/ASR/cognition, so the tradeoff is a ~15–30s cold-load cost per cycle in exchange for guaranteed full memory reclaim on exit. One camera integrates at a time, staggered by `occupancy.update_interval_seconds`. The whole pipeline can run at full resolution end to end: `CameraConfig`/`camera_discovery.capture_width`/`capture_height` opt a camera into requesting a higher native capture mode (e.g. true 4K — confirmed via `v4l2-ctl --list-formats-ext` that this rig's sensors expose 3840x2160 natively under their existing pixel format, so this deliberately does not force an unverified fourcc like MJPG), the occupancy pipeline encodes that frame losslessly for the depth model bounded only by `occupancy.max_input_width` (default 3840, distinct from and *not* the same as `vision.dashboard_max_width`'s lossy preview-stream cap), and `occupancy.process_res`'s ceiling is raised to admit true 4K if the deployment has the memory/GPU headroom for it — a 4K-capturing camera reaches the depth model undownscaled rather than being silently cropped to preview size first.

Per-voxel state is a log-odds occupancy estimate — the standard Bayesian occupancy-grid-mapping formulation used throughout robotics mapping (OctoMap, ROS `costmap_2d`) — not a binary flag: a direct depth hit nudges log-odds up (scaled by the depth model's per-pixel confidence), a ray traversal nudges it down, and evidence accumulates and clamps, so a single noisy observation barely moves an estimate while corroborated evidence dominates and sustained contrary evidence can still flip a voxel's classification (an object that moves away eventually reads as free again). Each occupied voxel also carries the real RGB color sampled from its source camera frame at the pixel that produced it, so the reconstruction reflects the actual observed scene rather than a synthetic confidence gradient. A compact natural-language summary (`summary_text()`) is written into the world model as `environment:egg`'s `occupancy_summary` property, available to LLM context the same way any other world-model fact is. Voxel size auto-scales with sampling density (`resolve_voxel_size_meters`): the dashboard's Resolution +/- control adjusts `sample_stride` (how many of the depth map's pixels get back-projected per cycle) and, together with it, the derived `voxel_size_meters` scaled proportionally — denser sampling resolves into finer voxels, coarser sampling into coarser ones — and rebuilds the grid fresh (existing voxel indices are keyed to the old size, so they'd decode to the wrong position under a new one) with every camera due again immediately rather than waiting out the old staggered per-camera schedule.

The `/vision` dashboard page renders this as an orbitable 3D scene of the voxel reconstruction alone (`services/vendor/occupancy_scene.js`) — no camera-preview sprites in the scene itself, just the fused geometry. It shares the `/graph` page's WebGL renderer rather than opening a second context — some browser/GPU configurations only sustain one live WebGL context per page — and transparently falls back to a pure 2D-canvas compatibility renderer (same rotation/projection technique as the knowledge-graph page's own fallback) when no WebGL context is available at all, so the view still renders on GPU-disabled or remote-desktop browser sessions. An "Egg POV" toolbar button locks the camera to the fused frame's origin — physically the rig's shared optical center — and switches dragging from orbiting a target to FPS-style look-around from that fixed point, implemented independently in both render paths.
...
The live model-authored capability contract can be exercised without microphone
input. `--execute-tools` performs the policy-gated web/page-read and read-only
shell cases as well as checking camera intent and explicit no-run conversation:

```bash
.venv/bin/python scripts/validate_realtime_capabilities.py --execute-tools
```

The bootstrap installs `egg-gpu-pm-guard.service` before display-manager and Ollama. The guard accepts the kernel's normal `auto` policy as well as `on`; the audit always probes CUDA directly and only treats explicit PM error/unsupported states as failures. Ollama is limited to one loaded model so ASR, cognition, and vision cannot exhaust unified memory. Its context window is not a fixed number: `scripts/size_ornith_context.py`, wired in as an `ExecStartPost` alongside the existing `repair_omnius_*` hooks, computes `num_ctx` from whatever memory is actually free each time Ollama starts and rewrites the model's own Modelfile to match, scaling gradually across the device's real operating range rather than reserving one worst-case allocation up front — a model-declared context far above the server default was found on this device consuming most of system memory before egg_companion's own processes ever got a chance to run. Below a floor of available memory it leaves the existing context size alone rather than risk a reload under pressure. `OLLAMA_NUM_PARALLEL` governs how many requests Ollama serves concurrently; it's conservative by default since a second slot costs its own KV-cache allocation, and should only be raised once there's confirmed memory headroom to spare.
